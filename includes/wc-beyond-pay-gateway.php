...
class WC_Beyond_Pay_Gateway extends WC_Payment_Gateway
{
    ...
    function admin_options()
    {
        ...
        wp_enqueue_script(
            ...
            plugins_url('assets/js/beyondpay-admin-options.js', dirname(__FILE__))
        );
    }
    ...
    public function payment_fields()
    {
        ...
        $form_event = isset($_GET['change_payment_method']) || is_add_payment_method_page(
        ) ? 'submit' : 'checkout_place_order';
        ?>
    ...
    public function process_payment($order_id)
    {
        ...
        $order                    = wc_get_order($order_id);
        $pay_with_token           = ! empty($_POST['wc-beyondpay-payment-token']) && is_numeric(
    ...
    {
        if ( ! empty($response->responseMessage->CardType) && ($token->get_card_type(
                ) === 'Card' || empty($token->get_card_type()))) {
    ...
    public function build_payment_request(
        ...
        if ($token) {
            if (is_string($token)) {
                ...
                $request->requestMessage->ExpirationDate = $token->get_expiry_month() . substr(
    ...
    public function add_payment_method()
    {
        ...
        if ($response->ResponseCode == '00000') {
            ...
            wc_add_notice(
                'Failed to add payment method: ' . $response->ResponseDescription . ' (' . $response->ResponseCode . ')',
                'error'
            );
    ...
    public function process_tokenized_payment($order_id)
    {
        ...
        if ( ! empty($tokens)) {
            try {
                ...
                $order->add_order_note(
                    'Failed to process payment, payment method connected to order no longer exists or is not valid. Internal error: ' . $error->getMessage(
                    )
                );
